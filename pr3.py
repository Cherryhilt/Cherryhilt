...
dataset = pd.read_csv('ICICI.csv', usecols=[4])
ClosePrice = dataset
...
def predict_price_LinearReg(dates, prices):
    ...
    next_day_index = np.array([len(prices)]).reshape(-1, 1)
    predicted_price = linear_mod.predict(next_day_index)[0][0]
    return predicted_price, xyz
...
print("-----Next Day Value Using ANN:", (last_val+next_val).item())
if last_val.item() < (last_val + next_val).item():
   print("\n----Price will go up Buy share----")
else:
   print("\n----Alert Price will go down----")


prices,dates= (getData('ICICI.csv'))
